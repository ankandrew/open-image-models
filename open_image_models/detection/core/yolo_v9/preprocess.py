...
def preprocess(img: np.ndarray, img_size: tuple[int, int] | int):
    ...
    im, ratio, (dw, dh) = letterbox(img, img_size, stride=32, auto=False, scale_fill=False)
...
def letterbox(
    ...
    """
    Taken from `ultralytics.utils.augmentations.letterbox`
    """
